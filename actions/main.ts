...
async function run(): Promise<void> {
    try {
        ...
        buffer = subProcess.execSync(`nohup sh -c "aws logs tail /sst/service/dev-serverless-mephisto-task-test-deployment-test-dev-wvpc-mephisto-task-test-deployment-test-dev-wvpc --follow --since 1d | egrep '${process.env.PREVIEW_URL_PREFIX}' > _run.log" &`);

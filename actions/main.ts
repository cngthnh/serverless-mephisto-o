...
async function run(): Promise<void> {
    try {
        ...
        info(`nohup sh -c "aws logs tail /sst/service/dev-serverless-mephisto-task-test-deployment-test-dev-wvpc-mephisto-task-test-deployment-test-dev-wvpc --follow --since 1d | egrep '${process.env.PREVIEW_URL_PREFIX}' --line-buffered > _run.log" &`);
        subProcess.execSync("touch _run.log")
        subProcess.spawn(`aws logs tail /sst/service/dev-serverless-mephisto-task-test-deployment-test-dev-wvpc-mephisto-task-test-deployment-test-dev-wvpc --follow --since 1d | egrep '${process.env.PREVIEW_URL_PREFIX}' --line-buffered > _run.log`);
